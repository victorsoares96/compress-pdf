...
// Copyright 1996-2003 Glyph & Cog, LLC (original XPDF implementation)
...
// ─── CCITTFaxDecoder ported from pdf.js ────────────────────────────────────
// Adaptation notes:
//   1. Removed `import` statements — replaced FormatError with Error, info with noop
//   2. Added explicit TypeScript types where needed
//   3. Logic kept byte-for-byte identical to the source

// eslint-disable-next-line @typescript-eslint/no-empty-function
function info(_msg: string): void {}
...
// prettier-ignore
const twoDimTable: [number, number][] = [
  [-1, -1], [-1, -1],                   // 000000x
...
// prettier-ignore
const whiteTable1: [number, number][] = [
  [-1, -1],                               // 00000
  [12, ccittEOL],                         // 00001
  [-1, -1], [-1, -1],                     // 0001x
...
// prettier-ignore
const whiteTable2: [number, number][] = [
...
// prettier-ignore
const blackTable1: [number, number][] = [
...
// prettier-ignore
const blackTable2: [number, number][] = [
...
// prettier-ignore
const blackTable3: [number, number][] = [
...
interface CCITTFaxDecoderOptions {
  K?: number;
  EndOfLine?: boolean;
...
class CCITTFaxDecoder {
  ...
  readNextChar(): number {
    ...
    let refPos: number, blackPixels: number, bits: number, i: number;
   ...
   * @private
   */
  _addPixels(a1: number, blackPixels: number): void {
    const codingLine = this.codingLine;
   ...
   * @private
   */
  _addPixelsNeg(a1: number, blackPixels: number): void {
    const codingLine = this.codingLine;
    let codingPos = this.codingPos;
   ...
   * @private
   */
  _getBlackCode(): number {
    let code: number, p: [number, number] | undefined;
    if (this.eoblock) {
