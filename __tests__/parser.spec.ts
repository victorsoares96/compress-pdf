...
describe(
  ...
  () => {
    ...
    it('throws CompressPdfError for wrong password', async () => {
      const bytes = await fs.promises.readFile(PROTECTED);
      await expect(loadPdf(bytes, 'wrongpass')).rejects.toThrow(
        CompressPdfError
      );
    });
...
describe(
  ...
  () => {
    ...
    it('saves with password when removePassword is false', async () => {
      const bytes = await fs.promises.readFile(PROTECTED);
      const doc = await loadPdf(bytes, 'a17');
      const saved = await savePdf(doc, { removePassword: false });
      await expect(loadPdf(Buffer.from(saved))).rejects.toThrow(
        CompressPdfError
      );
